...
local CracktroFrameText = "Clinty Reanimate V" .. UhhhhhhVersion
local UIMainWindow, WindowContent do
	...
	TopBarText.Text = "Clinty Reanimate | v" .. UhhhhhhVersion
...
task.spawn(function()
	...
	local AsciiTextarts = {
		...
		},
		{
			...
			"Believe me friend, its frickingly fast, you don't ev",
		},
		{
...
do
	...
	local CharacterScaleSlider = UI.CreateSlider(MainPage, "Character Scale", Reanimate.CharacterScale, 0.5, 5, 0.1)
	CharacterScaleSlider.Changed:Connect(function(val)
		val = math.clamp(val, 0.001, 10)
...
do
	...
	local NetVelSlider = UI.CreateSlider(MainPage, "Netless Velocity", Reanimate.NetlessVelocity, 1, 10000, 0.1)
...
SaveData.FavoriteDances = SaveData.FavoriteDances or {}

local DanceItemFrames = {}

local function IsDanceFavorited(m)
	GetModuleHash(m)
	for _, hash in SaveData.FavoriteDances do
		if hash == m.Hash then return true end
	end
	return false
end

local function SetDanceFavorite(m, fav)
	GetModuleHash(m)
	if fav then
		if not IsDanceFavorited(m) then
			table.insert(SaveData.FavoriteDances, m.Hash)
		end
	else
		for i, hash in SaveData.FavoriteDances do
			if hash == m.Hash then
				table.remove(SaveData.FavoriteDances, i)
				break
			end
		end
	end
...
local function SortDanceList()
	...
	table.sort(frames, function(a, b)
		local aFav = a:GetAttribute("IsFavorite") and true or false
		local bFav = b:GetAttribute("IsFavorite") and true or false
		if aFav ~= bFav then return aFav end
		return (a:GetAttribute("DanceName") or "") < (b:GetAttribute("DanceName") or "")
	end)
...
local function SortDanceList()
	local children = DancesPage.List:GetChildren()
	local frames = {}
...
local function AddDance(m)
	if type(m) == "table" then
		...
		local isFav = IsDanceFavorited(m)
		item.Parent:SetAttribute("IsFavorite", isFav)
		item.Parent:SetAttribute("DanceName", m.Name)

		-- Star button
		local StarBtn = Instance.new("TextButton", item.Parent)
		StarBtn.AnchorPoint = Vector2.new(1, 0)
		StarBtn.Position = UDim2.new(1, -4, 0, 4)
		...
		StarBtn.TextColor3 = isFav and Color3.fromRGB(255, 210, 0) or Color3.new(0.6, 0.6, 0.6)
		StarBtn.ZIndex = 5
		StarBtn.Name = "StarBtn"

		local function UpdateStar(fav)
			StarBtn.Text = fav and "★" or "☆"
			StarBtn.TextColor3 = fav and Color3.fromRGB(255, 210, 0) or Color3.new(0.6, 0.6, 0.6)
			item.Parent:SetAttribute("IsFavorite", fav)
		end

		StarBtn.Activated:Connect(function()
			local newFav = not IsDanceFavorited(m)
			SetDanceFavorite(m, newFav)
			UpdateStar(newFav)
			SortDanceList()
			Util.UINotify(newFav and ("★ Favorited: " .. m.Name) or ("Unfavorited: " .. m.Name))
		end)

		-- Keep star color synced with theme text color
		Util.LinkDestroyI2C(StarBtn, UITextColor.Changed:Connect(function()
			if not IsDanceFavorited(m) then
				StarBtn.TextColor3 = Color3.new(0.6, 0.6, 0.6)
			end
		end))
		...
		msname.Size = UDim2.new(1, 0, 1, 0)
		msdesc.Size = UDim2.new(1, 0, 1, 0)

		item.Parent.Name = m.Name .. " " .. m.Description
		DanceItemFrames[m.Hash] = item.Parent
		Util.LinkDestroyI2C(item, item.Activated:Connect(function()
			local page = UI.CreatePage()
			page.ZIndex = 2
			page.Position = UDim2.new(0.5, 360, 0.5, 0)
